...
public class Database {
	...
	protected void appendPage(StringBuilder builder, int pageSize,
			int pageOrdinal) {
		throw new RuntimeException(
				"Super unfortunate: Database paging and limits are vendor specific!");
	}

	protected DatabaseCommand buildSelectCommand(EntityAspect aspect,
			EntityFilter filter, EntitySortNode sort, int pageSize,
			int pageOrdinal) {
	...
	protected DatabaseCommand buildSelectMemberCommand(EntityAspect aspect,
			int memberOrdinal, EntityFilter filter, EntitySortNode sort,
	...
	protected DatabaseCommand buildDeleteCommand(EntityAspect aspect,
			EntityFilter filter) {
		DatabaseCommand cmd = new DatabaseCommand(255);
	...
	protected final DatabaseCommand buildUpdateCommand(EntityAspect aspect,
			Object[] values, EntityFilter filter) {
	...
	protected DatabaseCommand buildInsertCommand(EntityAspect aspect,
			Object[] values, boolean scopeIdentity) {
		...
		cmd.text.append(" ( ");
		for (int i = 0; i < values.length; i++)
			if (values[i] != Void.class && aspect.get(i).columntType != ColumnType.IDENTITY) {
				if (prependComma) {
					cmd.text.append(", ");
	...
	protected DatabaseCommand buildSelectExists(EntityAspect aspect,
			EntityFilter filter) {
	...
	protected DatabaseCommand buildSelectCount(EntityAspect aspect,
			EntityFilter filter) {
		DatabaseCommand cmd = new DatabaseCommand(155);
